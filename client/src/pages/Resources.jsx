...
import { statusBadgeClass, statusLabel, getCurrentUSDRate, fmtUSD, fmtRate, getInitials } from '../lib/costEngine';
      ...
      <div className="section-header">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <select className="form-select" style={{ width: 'auto' }} value={fType}   onChange={e => setFType(e.target.value)}>
            <option value="">All Types</option><option value="FT_EMPLOYEE">FT Employee</option><option value="PT_EMPLOYEE">PT Employee</option><option value="CONTRACTOR">Contractor</option><option value="C2C">C2C</option>
          </select>
          <select className="form-select" style={{ width: 'auto' }} value={fStatus} onChange={e => setFStatus(e.target.value)}>
            <option value="">All Status</option><option value="DEPLOYED">Deployed</option><option value="PARTIALLY_DEPLOYED">Partial</option><option value="AVAILABLE">Available</option><option value="ON_BENCH">On Bench</option>
          </select>
                ...
                {resources.map(r => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td><span className="badge badge-gray">{r.employmentType?.replace('_',' ')}</span></td>
                      <td>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        ...
                        <button className="btn btn-outline btn-xs" style={{ marginLeft: 3 }} onClick={() => openEdit(r)}>Edit</button>
                      </td>
                    </tr>
                  );
                })}
