    ...
    <div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <span style={{ fontSize:13, color:'var(--muted)' }}>{projects.length} project{projects.length !== 1 ? 's' : ''}</span>
        </div>
    ...
    <div>
      ...
      <div className="card">
        <div className="tabs">
          {['overview','roles','milestones','actuals'].map(t => (
            <div key={t} className={`tab ${activeTab===t?'active':''}`} onClick={() => setActiveTab(t)}>
              {t==='overview'?'Overview':t==='roles'?`Roles & Deployments (${project.roles?.length||0})`:t==='milestones'?'Milestones':'Monthly Actuals'}
            </div>
          ))}
        </div>
            ...
            <div className="form-grid-2">
              <div>
                ...
                {[
                  ...
                  ['Type',           project.sowType==='FIXED'?'Fixed Price':'T&M'],
              ...
              <div>
                ...
                {project.notes && (
                  <div style={{ marginTop:12, padding:10, background:'var(--surface2)', borderRadius:8, fontSize:12, color:'var(--text2)', lineHeight:1.6 }}>
                    {project.notes}
                  </div>
                )}
              </div>
        ...
        {activeTab === 'roles' && (
          <div className="card-body">
            ...
            {(project.roles || []).length === 0 ? (
              ...
              <RoleCard key={role.id} role={role} project={project} resources={resources} skills={skills} projectId={projectId} onRefresh={refresh} />
            ))}
          </div>
        )}
        ...
        {activeTab === 'milestones' && (
          <div className="card-body">
            {project.sowType !== 'FIXED' ? (
              <div className="info-box">Milestones are for Fixed Price SOWs. This is a T&M engagement.</div>
            ) : (
              <>
                <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:12 }}>
                  <button className="btn btn-primary btn-sm" onClick={() => setShowAddMS(true)}>+ Add Milestone</button>
                </div>
                <MilestonesTab project={project} onRefresh={refresh} />
              </>
            )}
          </div>
        )}
        ...
        {activeTab === 'actuals' && (
          <div className="card-body">
            ...
            {(project.roles || []).flatMap(r => r.deployments || []).length === 0 && (
              <div className="empty-state"><div className="empty-icon">📊</div><div className="empty-text">No deployments yet. Assign resources to roles first.</div></div>
            )}
          </div>
        )}
      </div>

      {showAddRole && <AddRoleModal project={project} skills={skills} onClose={() => setShowAddRole(false)} onSaved={() => { setShowAddRole(false); refresh(); }} />}
      {showAddMS   && <AddMilestoneModal projectId={projectId} onClose={() => setShowAddMS(false)} onSaved={() => { setShowAddMS(false); refresh(); }} />}
      {showEdit    && <SOWFormModal project={project} onClose={() => setShowEdit(false)} onSaved={() => { setShowEdit(false); refresh(); }} />}
    </div>
  );
}
...
function RoleCard({ role, project, resources, skills, projectId, onRefresh }) {
  ...
  const ef = editForm || {
    title: role.title, skillId: role.skillId||'', billRate: role.billRate||'',
    billingType: role.billingType, fixedAmount: role.fixedAmount||'',
    planStart: role.planStart?.split('T')[0]||'', planEnd: role.planEnd?.split('T')[0]||'',
  };
  ...
  return (
    <div style={{ border:'1px solid var(--border)', borderRadius:10, marginBottom:12, overflow:'hidden' }}>
        ...
        {!editing ? (
          <>
            ...
            <div style={{ display:'flex', gap:6 }}>
              ...
              <button className="btn btn-accent btn-xs" onClick={() => setShowAssign(true)}>+ Assign Resource</button>
              <button className="btn btn-danger btn-xs" onClick={() => { if (window.confirm(`Mark role "${role.title}" as inactive?`)) deleteMut.mutate(); }}>✕</button>
            </div>
          </>
        ) : (
          <div style={{ width:'100%' }}>
      ...
      {(role.deployments || []).length === 0 ? (
        <div style={{ padding:'10px 14px', fontSize:12, color:'var(--muted)', fontStyle:'italic' }}>No resources assigned yet — click "+ Assign Resource"</div>
      ) : (role.deployments || []).map(dep => {
        const res  = resources.find(r => r.id === dep.resourceId);
        const rate = res ? getCurrentUSDRate(res) : 0;
        const hrs  = 21 * 8 * (dep.allocation / 100);
      ...
      {showAssign && (
        <AssignResourceModal role={role} project={project} resources={resources}
...
function MilestonesTab({ project, onRefresh }) {
  ...
  if ((project.milestones || []).length === 0) {
    return <div className="empty-state"><div className="empty-icon">🏁</div><div className="empty-text">No milestones yet.</div></div>;
  }
          ...
          return (
            <tr key={m.id}>
              ...
              <td>
                ...
                <button className="btn btn-danger btn-xs" onClick={() => { if (window.confirm(`Archive milestone "${m.name}"?`)) deleteMut.mutate(m.id); }}>✕</button>
              </td>
            </tr>
          );
        })}
...
function ActualsRow({ dep, role, resources, projectId }) {
  ...
  const plannedHrsPerMonth = 21 * 8 * (dep.allocation / 100);
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:8 }}>
        {months.map(m => (
          <div key={m.key} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, padding:'9px 10px' }}>
            ...
            <input type="number" step="0.5" placeholder={plannedHrsPerMonth.toFixed(0)}
              defaultValue={m.existing?.actualHours||''} className="form-input"
              style={{ padding:'4px 6px', fontSize:11, marginBottom:4 }}
            ...
            <div style={{ fontSize:10, color:'var(--muted)' }}>
              {m.existing
                ? <span style={{ color:'var(--accent)' }}>✓ {fmtUSD(m.existing.actualHours * rate)}</span>
                : <span>Plan: {fmtUSD(plannedHrsPerMonth * rate)}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="modal-box modal-box-lg">
        ...
        <div className="form-grid-3">
          <div className="form-group">
            ...
            {pmUsers.length===0 && <div className="form-note">Add PM users in Settings → Users</div>}
          </div>
          <div className="form-group">
      ...
      <div className="modal-box">
        ...
        <div className="form-group"><label className="form-label">Resource *</label>
          <select className="form-select" value={form.resourceId} onChange={e=>f('resourceId',e.target.value)}>
            ...
            {resources.filter(r=>r.status!=='EXITED').map(r=>(
              <option key={r.id} value={r.id}>{r.name} · {r.primarySkill?.name} · {fmtRate(getCurrentUSDRate(r))}/hr</option>
            ))}
          </select>
        </div>
